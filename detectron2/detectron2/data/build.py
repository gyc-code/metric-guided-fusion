...
def filter_images_with_only_crowd_annotations(dataset_dicts):
    ...
    # dataset_dicts = [x for x in dataset_dicts if valid(x["annotations"])]# cindy comment, original
    if 'source' in dataset_dicts[0]:
        dataset_dicts = [x for x in dataset_dicts if valid(x['source']["annotations"])]
    else:
...
def filter_images_with_few_keypoints(dataset_dicts, min_keypoints_per_image):
    ...
    if 'source' in dataset_dicts[0]:
        dataset_dicts = [
            x for x in dataset_dicts if visible_keypoints_in_image(x['source']) >= min_keypoints_per_image
        ]
    else:
        dataset_dicts = [
            x for x in dataset_dicts if visible_keypoints_in_image(x) >= min_keypoints_per_image
...
def print_instances_class_histogram(dataset_dicts, class_names):
    ...
    for entry in dataset_dicts:
        ...
        if 'source' in entry:
            annos = entry['source']["annotations"]
        else:
            annos = entry["annotations"]
...
def get_detection_dataset_dicts(
    ...
    # cindy : to process uda data
    if 'source' in dataset_dicts[0]:
        has_instances = "annotations" in dataset_dicts[0]['source']
    else:
