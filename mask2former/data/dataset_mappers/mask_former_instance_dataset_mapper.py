...
class MaskFormerInstanceDatasetMapper:
    ...
    def __mask_to_instance__(image, annos, masks):
        ...
        if "source" in dataset_dict:
            for data_key in dataset_dict.keys():
                ...
                if data_key == "source":
                    ...
                    source_far_region = depth_map > 70
                    true_indices = np.where(source_far_region)
                    ...
                    source_far_region = depth_map < 30
                    true_indices = np.where(source_far_region)
                    max_row_index = np.max(true_indices[0])
                ...
                cv2.imwrite('source_' + dataset_dict[data_key]["image_id"] + '_far.png',((source_far_region*1)*255))
                ...
                for index, t in enumerate(far_region_transforms):
                    ...
                    if 'Resize' in t.__class__.__name__:
                        continue
                    far_region_image = t.apply_image(image)
                ...
                if data_key == "target":
                    ...
                    far_region_annos, far_region_masks = self.__transform_annotation__(far_region_transforms, dataset_dict, data_key, far_region_image)
                    # assert "annotations" in dataset_dict[data_key]
                    # for anno in dataset_dict[data_key]["annotations"]:
                    #     anno.pop("keypoints", None)
                    ...
                    instances = self.mask_to_instance(image, annos, masks)
                    far_region_instances = self.mask_to_instance(far_region_image, far_region_annos, far_region_masks)
                    # classes = [int(obj["category_id"]) for obj in annos]
                    # classes = torch.tensor(classes, dtype=torch.int64)
                    # image_shape = (image.shape[-2], image.shape[-1])  # h, w
                    # # Pytorch's dataloader is efficient on torch.Tensor due to shared-memory,
