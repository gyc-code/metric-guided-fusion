...
# ========================== PCA 标量图（可视化） ==========================
def _scalar_map_from_feat_pca_simple(
    Fmap: torch.Tensor,
    take_abs: bool = False,
    pool_down: int = 1,
    n_components: int = 1,
...
def compute_FCD_from_features(
    ...
    img = _prep_to_bchw(image_chw_uint8, (H,W), dev)
    if img.shape[0] != B: img = img.expand(B,-1,-1,-1)
    Y = _rgb_to_luma_Y(img)  

    GI, _, _ = _sobel_replicate(Y)                   # (B,1,H,W)
    GI = _minmax_in_mask(GI.squeeze(1), valid)       # (B,H,W)

    # 2) PCA 标量图 M ——保存
    M = _scalar_map_from_feat_pca_simple(
        feat, n_components=1, upsample_to=(H,W)
    ).clamp(0,1)                                     # (B,H,W)
    ...
    # print("fcd time cost:", time.time() - start)
...
if __name__ == "__main__":
    ...
    fcd = compute_FCD_from_features(
        feat_flat, imgs, q_top=0.10, q_img=0.10, dilate_r=2
    )
